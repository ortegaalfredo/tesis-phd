...
\begin{figure}[t]
  ...
  \caption{Comparación gráfica entre los métodos de acceso múltiple: TDMA (\textit{Time Division Multiple Access}), FDMA (\textit{Frequency Division Multiple Access}) y CDMA (\textit{Code Division Multiple Access})}
  \label{fig_tdmacdma}
\end{figure}
...
Se denominan códigos de corrección de errores asimétricos o unidireccionales, a todo código de corrección especializado en canales de comunicaciones cuya probabilidad de error no se aplica simétricamente a todos los símbolos. Estos métodos son de especial interés en esta Tesis, debido a que las modulaciones utilizadas en ambos medios seleccionados, el acústico y el óptico, poseen un modelo de ruido esencialmente asimétrico.
Un caso sencillo, es por ejemplo, el del llamado canal Z, un canal binario (símbolos ``1'' y ``0'') pero cuyos errores sólo pueden afectar a uno de los símbolos, como podría suceder en un sistema donde al transmitir el símbolo ``0'' un error provoque la recepción de un símbolo ``1'', pero donde lo contrario no es posible. En la figura~\ref{fig:Gal} se observa el diagrama de probabilidades del canal binario asimétrico o canal Z. En la sección~\ref{canalZ} se desarrolla el límite de capacidad del canal Z.
...
\subsection{Sistemas de comunicacion óptica y soluciones relacionadas}
\label{defPON}
