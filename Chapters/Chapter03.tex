...
Reemplazando \ref{eq:Pnc0} en \ref{eq:PncSym}:
...
$P_{0}$ como la probabilidad de decodificar un bit en cero, como un bit en uno. O sea, el BER.

La ecuación \ref{eq:Pnc}, define la posibilidad de colisión de un cliente con cualquier otro. Ahora podemos asumir que en la trama, existiran $W_1 = N\cdot K\cdot m_1$ unos. Por lo tanto, la posibilidad de colisión $P_{c0}$ de un bit en cero con cualquier bit en uno es:

\begin{equation}
\label{eq:Pnc1}
P_{c0}=1-\left(1-\frac{1}{M}\right)^{N\cdot K\cdot m_1}
\end{equation}

Debido a que el filtro de Bloom mutiplica la cantidad de unos por $K$, y cada símbolo posee en promedio $m_1$ unos.
Ahora, debido también al filtro de Bloom, para que se produzca un error es necesario que todos los $K$ ceros transmitidos colisionen con unos. Esto reduce la probabilidad de error, y puede representarse como la probabilidad de error post-filtro de Bloom $P_{bf}$ de esta manera:

\begin{figure}[!t]
  \centering
    \includegraphics[width=5in]{graphs/calcK}
    \caption{Estimación de BER vs. tasa de repetición de filtro de Bloom K. Se observan distintas curvas correspondientes a diferentes pesos de Hamming. Puede apreciarse una reducción considerable del BER estimado a medida que el peso de Hamming disminuye.}
    \label{BERvsK}
\end{figure}

\begin{equation}
\label{eq:Pnc2}
P_{bf}=\left(1-\left(1-\frac{1}{M}\right)^{N\cdot K\cdot m_1}\right)^K
\end{equation}

Podemos tomar la probabilidad $P_{bf}$ como una estimación del BER del sistema. Finalmente, al aplicar la codificación de reducción del peso de Hamming en \ref{eq:Pnc2}, el efecto es la reducción del parámetro $m_1$. Nótese que la cantidad de bits en cero no afectan esta ecuación, por lo que la expansión en largo del símbolo no tiene ningun efecto.

La figura \ref{BERvsK} muestra la estimación del BER en función de $K$ para $M=4096$, $m_{1}$ varía de 2 a 4 y $N=128$. Es interesante notar que existe un valor óptimo de la tasa de repetición $K$ que minimiza la tasa de error. Este resultado es relevante en el diseño de un sistema de comunicaciones. Debido a los bajos valores de BER alcanzados, es posible utilizar en la siguiente etapa algoritmos eficientes que aceptan bajos límites de error, tal como Reed-Solomon, para reducir aún mas el BER total de sistema y asi llevarlo a tasas aceptables para aplicaciones prácticas.
